...
import { censusInfo, sceneEventParams } from "@/interfaces/rule.interface"
...
export default {
	...
	barrageSubmit(params: { content: string; view_name: string }) {
		return postJson<baseRes<barrageContentItem>>({
			url: `${urlFunc.requestHost("node")}/v1/front/barrage/submit`,
			params
		})
	},
	barrageList(params: { limit: number; offset: number; view_name: string }) {
		return postJson<baseRes<barrageContentList>>({
			url: `${urlFunc.requestHost("node")}/v1/front/barrage/list`,
			params
		})
	},
	newestbarrage(params: { id: number }) {
		return postJson<baseRes<barrageContentItem>>({
			url: `${urlFunc.requestHost("node")}/v1/base/barrage/checkBarrage`,
			params
		})
	},
	//默认弹幕列表
	defaultBarrage(params: { limit: number; offset: number }) {
		return postJson<baseRes<barrageContentList>>({
			url: `${urlFunc.requestHost("node")}/v1/base/defaultBarrage/listState`,
			params
		})
	},

	addMessage(params: { content: string; name: string; view_name: string }) {
		return postJson<baseRes<barrageContentItem>>({
			url: `${urlFunc.requestHost("node")}/v1/base/message/add`,
			params
		})
	},
	messageList(params: { limit: number; offset: number; view_name: string }) {
		console.log(params, "params")
		return postJson<baseRes<barrageContentList>>({
			url: `${urlFunc.requestHost("node")}/v1/base/message/liststate`,
			params
		})
	},
	unzip(params: { url: string }) {
		console.log(params, "params")
		return postJson<baseRes<barrageContentList>>({
			url: `${urlFunc.requestHost("node")}/v1/base/system/unzip`,
			params
		})
	},
	//点赞
	sceneCommit(params: sceneEventParams) {
		return get<baseRes>({
			url: `${urlFunc.requestHost("material")}/v1/api/index/scene/commit/new`,
			params
		})
	},

	// 点赞总数信息
	censusInfo(params: { sceneName: string }) {
		const { sceneName } = params
		return get<baseRes<censusInfo>>({
			url: `${urlFunc.requestHost("material")}/3d/${sceneName}/temp/info.json`
			// params
		})
	}
}
