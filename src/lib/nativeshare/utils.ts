...
function loadJs(src: string, callback = noop) {
...
function assign(target: Object = {}, varArgs: Object = {}) {
	if (target == null) {
...
function openAppByScheme(scheme: string) {
	if (isIos) {
...
function generateQueryString(queryObj: Object = {}, needEncode = false) {
	const arr = []
...
const Base64 = {
	...
	_utf8_encode: function (a: string) {
		a = a.replace(/\r\n/g, "\n")
...
function getHrefFromIconTag() {
	return iconTag?.getAttribute("href") || `${location.protocol}//${location.hostname}/favicon.ico`
}
...
function setDescTagContent(content: string) {
	if (descTag) {
...
function setIconTagHref(href: string) {
	if (iconTag) {
...
function setTitleTagTitle(title: string) {
