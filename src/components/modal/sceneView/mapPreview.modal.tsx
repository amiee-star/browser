...
const MinMap = () => {
  ...
  const getPreview = useCallback((item) =>(e: { stopPropagation: () => void })=>{
    // e.stopPropagation()
    // ModalCustom({
  ...
  return (
    <div className="map-box" ref={mapBox} style={{height:603}}>
      {!imgData.sourceWidth ? (
        loadEle
      ) : (
        <animated.div
...
const PreviewMapModal: React.FC<Props & ModalRef> = props => {
	const { data, onClose, modalRef, url } = props
	...
	return (
    <>
		<div id={"PreviewMapModal"}>
      <MinMap />
      <Button type="link" shape="circle" size="large" icon={<CloseCircleOutlined style={{fontSize:35}} />} onClick={modalClose}  />
		</div>
    </>
	)
}
